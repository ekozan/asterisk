> ## ⚠ Implémentation archivée
>
> Ce répertoire contient une implémentation sur mesure qui **n'est plus le chemin retenu**.
> Le PBX du projet est désormais FreePBX — voir le [README](../README.md) et
> [docs/](../docs/) à la racine.
>
> Elle reste ici parce qu'elle fonctionne, qu'elle est testée, et surtout parce que sa
> documentation explique le *pourquoi* de décisions qui restent vraies sous FreePBX : la
> fraude à la tonalité, le contexte `[default]` laissé vide, la détection du raccroché sur
> un port FXO, la segmentation réseau, le nommage des objets `aor`.
>
> Tout ce qui suit décrit cette implémentation-là. Les chemins (`scripts/`, `docs/`, `app/`)
> sont relatifs à ce répertoire.
